        ...
        *,
        ...
        try:
            for index, step in enumerate(steps, start=1):
                ...
                current_context.state["progress"] = index / max(len(steps), 1)
                self._state_manager.set_task_state(f"plan.{plan_id}.context", current_context.to_result())
                self._state_manager.set_task_state(f"plan.{plan_id}.progress", current_context.state["progress"])
        ...
        current_context.state["status"] = "done"
        self._state_manager.set_task_state(f"plan.{plan_id}.result", current_context.to_result())
        self._state_manager.set_task_state(f"plan.{plan_id}.status", "done")
        return current_context
