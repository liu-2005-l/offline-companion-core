...
- P3-A4/A5 最终门禁通过：A4 专测 `16 passed`、persona fixture 全集 `191 passed`、运行时受影响链 `189 passed`、全量 `1444 passed, 3 skipped`；未验证人格即时流式与保护区逐字节透传保持不变。
...
- 冻结五人格 × 三类共 15 条 reply 文案并纳入六源哈希链；保护区继续使用既有确定性输出，不进入模型后端或 L1 prompt。
- 收敛 persona 写入与创建器派生：active SQL 改走不提交的 repo setter，前端删除 traits/desc/anchor 第二套规则，预览与保存共用本地后端纯派生函数，外部 traits 写入返回 `traits_read_only`。
- P3-A2 收尾门禁通过：`pytest -q` 为 1389 passed、3 skipped；T12-T26、D-A6 预览零副作用与预览/保存一致性均闭合。
- P3-A1.1 补强 persona 会话持久化闭合门：双真实进程恢复、注册表驱动 session holder 重绑、Consent pending 随旧会话挂起、幂等键跨重启复验，以及快照 source 三值枚举与旧值哈希迁移。
- 完成拟人表述 W2 三臂受控矩阵、F2 出口防线复检与 25 组匿名盲评；A/B/C 均不进入发布，避免把未过身份漂移、跨轮复读或体验优势 gate 的实验能力包装为 v1.9.0 功能。
- 锚定人格约束体系 P 批 v1.5：以 OCEAN 数值数组为事实源，冻结四项启动锚点、三档派生切点、traits 用户数据迁移义务、W3 编号及 persona 新会话切换契约。
- 冻结人格约束 P2 二轮形态预实验：以 GGUF chat template 的真实历史渲染为变量，纠正首轮比分与 L4 fixture 口径，预注册 338 轮矩阵、短前缀复制 gate、有效性哨兵及穷尽降级裁决树。
- 完成人格约束 P2 二轮 338 轮矩阵：F1b/F2 均因禁用语义族、示例复制、方向与人工自然度未过而否决，按预注册分支 3 选择 F0b 已知风险降级载体并解除 P2 语料创作入口；伴随发现 user 示例内容泄漏进入 P3/P4 复验。
- 闭合人格约束 P2 语料批：终裁 15 档位 30 段维度对话、三类 9 条结构样本与五人格组合，以单入口 YAML 清单保持事实源唯一；静态 gate 通过不冒充运行时证明，F0b 泄漏、身份断崖与示例复制继续转 P3/P4 复验。
- W2 runner 新增逐 arm/kind/seed checkpoint、断点续跑与跨平台单实例锁，防止本地模型并行推理污染单变量归因。
- F2-c 全量 370 条复检确认身份检测与 4-gram 复读度量正交：366 条干净通过，4 条非身份输入轮触发且人工定性均为真阳性；现有输入侧触发门仅覆盖 8/370，输出侧全量检测与扩展风险域进入 W3。
