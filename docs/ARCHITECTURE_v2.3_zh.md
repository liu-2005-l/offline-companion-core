...
> **英文**：[`ARCHITECTURE_v2.0_en.md`](./ARCHITECTURE_v2.0_en.md)（文件名保持 v2.0，内容待同步）
...
**B/C 层通过统一消息协议与 A 层通信**，仅发送/接收标准化消息（BaseMessage），不关心传输层协议。所有跨层通信统一使用 BaseMessage 格式（含 `role`、`content`、`meta`、`timestamp`），无论底层传输是函数调用、HTTP 还是消息队列，消息结构保持一致。`content` 字段支持 `text`、`image_url`、`audio_buffer` 等类型。当前 MVP 仅实现 `text`，其余类型为协议层占位，供后续多模态 Skill 接入。

消息总线是实现层概念，不是协议层概念。协议层统一使用 BaseMessage 格式，实现层按需选择：
- **核心对话链路**：走同步实现（函数调用模拟，符合 BaseMessage 协议），保证延迟和可调试性
- **后台任务链路**：走异步实现（ZeroMQ 消息队列，同样符合 BaseMessage 协议），不阻塞主对话

B/C 层只面向 BaseMessage 协议，不感知底层是同步还是异步。协议统一，实现分离。
...
| 面 | 职责 | 包含模块 | 约束 |
|----|------|----------|------|
| **控制面** | 策略决策与意图判断 | Policy、Consent、Router LLM、PlanOrchestrator、GoalManager、AttentionAwareness | 不直接操作数据存储，决策结果写入 message 专属字段 |
...
- **底层隔离强制**：状态域按域拆分独立 SQLite 文件，所有读写必须经 StateManager DAO 层，CI 拦截直连数据库操作，版本号校验由数据库触发器实现。
...
**设置页安全标识**：所有 Native 模式 Skill 在列表中常驻橙色「无隔离」标签，底栏常驻安全状态提示。CubeSandbox 模式 Skill 显示绿色「硬件隔离」标签，Docker 模式显示蓝色「进程隔离」标签。
...
**可选交付（时间充裕再做）**：CubeSandbox 可行性验证、响应格式全量对齐
...
**CubeSandbox 最小可行性验证**（Sprint 7 第一周）：只测三个核心指标——跨平台启动速度、内存占用、基础逃逸测试。验证不通过，直接降级为远期规划，所有 CubeSandbox 特性从主文档移入附录，后续不再投入核心资源。
