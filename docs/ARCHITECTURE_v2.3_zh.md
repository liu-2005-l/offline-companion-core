...
| StateManager | SQLite + 内存缓存，只存状态不做业务逻辑；基础三域封装已落地（session/task/system） | 🔶 部分完成 |
...
> 注：该能力当前仍处于排期与设计收拢阶段，未在 Sprint 7 代码中落地。
...
Codex 卡片；樱花粉。本文档自包含 `purpose_type` 全量定义，所有出站、降级、插件高危调用、沙箱风险提示均走统一审计表。
...
**说明**：`purpose_type` 统一采用一次性审计，不做跨会话复用；所有条目均记录 `created_at`、`trace_id`、`actor`。
...
| 依赖管理 | 声明步骤间的数据依赖（Step 2 需要 Step 1 的输出） | 📅 S9A |
| 状态追踪 | 每个步骤的状态：pending / running / done / failed | 📅 S9A |
| 错误恢复 | 步骤失败时的重试策略或降级路径；每个步骤可声明 `idempotent`（是否幂等），幂等步骤重试时复用已有结果，非幂等步骤跳过直接重试或要求用户确认 | 📅 S9A |
| TaskContext | 临时数据空间，Skill 执行时读写，任务完成后可选写入 B2 记忆 | 📅 S9A |
| 与 A3 关系 | 调用 Skill 时仍走 A3 Consent，B/C 层不感知计划存在 | 📅 S9A |
| **CubeSandbox 快照增强（可选）** | 仅 CubeSandbox 模式可用，执行每个步骤前做快照，失败可回滚。基础模式依赖 TaskContext + 幂等步骤实现错误恢复，不依赖沙箱快照特性 | 📅 S9A |
...
| 进度评估 | 定期评估目标进度（何时检查，检查什么） | 📅 S9B |
| 触发提醒 | 检测到触发条件时，通过 B1 装配 prompt 主动提及 | 📅 S9B |
| 频率控制 | 主动提醒频率硬上限（如每小时最多一次），用户可关闭 | 📅 S9B |
| 提醒决策 | 引入效用函数：Utility = Value_of_Reminder - Cost_of_Distraction。低效用提醒在用户专注时自动降级（弹窗 → 托盘图标闪烁），高效用提醒（用户显式设定的紧急提醒）不受限制 | 📅 S9B |
| 负反馈三级判定 | ① 强负反馈：用户关闭通知、语义明确拒绝（复用 B3 层检测能力）；② 弱负反馈：用户划掉通知、不回复；③ 正反馈：用户回复感谢、点击查看。每级对应明确权重系数，语义负反馈做关键词+语义双重判断，不能只靠关键词匹配 | 📅 S9B |
| 紧急提醒仅用户标记 | 禁止模型/效用函数自行判断「紧急」，只有用户手动标记为「紧急」的待办/提醒才可突破静默期 | 📅 S9B |
...
| 项 | 共识 | 状态 |
|----|------|------|
| 定时任务 | Cron-like，如「每天早上 8 点备份数据」 | 📅 S8 |
| 延迟任务 | 如「5 分钟后提醒我开会」 | 📅 S8 |
| 事件监听任务 | 如「监控某个网页价格变化」 | 📅 S8 |
| 长时间运行任务 | 如「帮我下载这个 50GB 的文件」 | 📅 S8 |
| Skill 集成 | Skill 通过标准 API 向 JobScheduler 注册后台任务 | 📅 S8 |
| 持久化 | 任务状态持久化到 SQLite，Agent 重启后可恢复 | 📅 S8 |
| UI 不阻塞 | 所有后台任务异步执行，通过事件通知 UI（完成/失败/进度更新） | 📅 S8 |
| Skill 存活检查 | 触发任务前先通过 skill_manager 检查目标 Skill 是否运行；已停止则标记为 failed，生成 `source: TOOLBOX` 错误记录 | 📅 S8 |
| 任务心跳 | 每个运行中的任务定期更新心跳时间戳；Agent 重启后检查心跳超时的任务，标记为 `failed` 或 `retry` | 📅 S8 |
| 调度算法 | 首批采用队列式（FIFO），后续版本评估抢占式优先级调度的必要性 | 📅 S8 |
| 暂停恢复条件 | ResourceArbitrator 解除资源紧张状态后，JobScheduler 按暂停时间顺序恢复任务（先暂停的先恢复） | 📅 S8 |
...
| B3 | `E_POLICY_DENIED` | B3 | 否 | 当前请求触发安全策略，已阻断。 |
| B3 | `E_CONSENT_REQUIRED` | B3 | 是 | 该操作需要您的确认。 |
| B3 | `E_LOCAL_ONLY_DENIED` | B3 | 否 | 当前为本地模式，无法执行该请求。 |
...
- **已落地基础能力**：连续失败计数 + 熔断态拦截 + `CircuitBreakerOpenError` 基础返回已实现；半开探测与恢复窗口仍待补全 | 🔶 部分完成
...
| 项 | 共识 | 状态 |
|----|------|------|
| 静默期检测 | 通过 agent-toolbox 获取前台应用状态（全屏？游戏？视频会议？）；深夜时段自动降低提醒等级 | 📅 S9B |
| 提醒分级 | ① 低优先级：仅侧边栏气泡或托盘图标变色；② 中优先级：系统原生通知；③ 高优先级：弹窗+声音（仅用户显式设定的紧急提醒） | 📅 S9B |
| 抑制输出 | 用户忙碌时 IdleThink 产生的提醒不入 B1 装配，写入待办队列，空闲时呈现 | 📅 S9B |
| 隐私边界 | 启用需显式独立 Consent，弹窗告知「此模组需要读取您的系统活动状态（如是否全屏、是否在输入文字），用于判断是否适合发送提醒。不会记录或上传任何屏幕内容。」；设置页提供独立开关，关闭后不影响其他 IdleThink 能力 | 📅 S9B |
| 场景静默 | 检测到全屏程序（游戏、视频会议）、深夜时段（23:00-7:00），静默所有非紧急提醒，降级为托盘红点 | 📅 S9B |
| 频率硬锁 | 主动提醒每小时 ≤1 次，每天 ≤3 次，用户可调整但上限不可突破 | 📅 S9B |
| 负反馈硬降权 | 用户关闭/忽略同类型提醒 2 次 → 7 天内不触发；用户回复中检测到「别提醒了」「吵」「不用管」等语义负反馈（复用 B3 层检测能力），同步生效 | 📅 S9B |
...
| 项 | 共识 | 状态 |
|----|------|------|
| 前台优先 | C1 生成长文本时，自动暂停 JobScheduler 中 `low_priority` 后台任务 | 📅 S9 |
| 沙箱资源限制 | CubeSandbox 模式：单实例 <5MB，无显式资源限制；Docker 模式：`--memory=1g --cpus=1`（基线：Playwright + Python 解释器 ≈ 800MB） | 📅 S9 |
| 并发上限动态调整 | 根据当前沙箱模式动态调整并发上限：CubeSandbox 模式最多 20 个并发实例，Docker 模式最多 3 个，Native 模式最多 1 个 | 📅 S9 |
| 降级策略 | 可用内存低于阈值（<500MB）时 IdleThink 自动暂停，托盘图标显示「资源紧张」 | 📅 S9 |
| JobScheduler 预留 | S8 的 JobScheduler 每个任务带 `priority` 和 `resource_requirement` 字段 | 📅 S8 |
| 硬件基准测试 | Sprint 8 补充硬件基准测试，确定各档位合理阈值；GPU 显存通过 `nvidia-smi` / `rocm-smi` 精准获取 | 📅 S8 |
| 临时默认值 | 内存阈值默认 500MB，显存阈值默认 2GB（保守值，覆盖大多数 8GB 显卡 + 7B 模型的场景），Sprint 8 硬件基准测试后调整 | 📅 S8 |
...
| 项 | 共识 | 状态 |
|----|------|------|
| 开发者模式 | 桌面壳增加「开发者模式」开关，打开后侧边栏新增「消息监视器」，实时显示 A/B/C 层流转的 JSON 消息 | 📅 S8 |
| Skill 拖拽编排 | 开发者模式下支持 Skill 拖拽编排——用户直接在 UI 上把「输入 → 技能A → 技能B → 输出」连起来，生成组合 Skill | 📅 S8 |
| 自动化评测 | `Evaluator` 组件：每次 IdleThink 或 GoalManager 做出决策后，自动记录「预期结果」和「实际结果」，量化「主动关心是否变成了骚扰」 | 📅 S8 |
| `check_imports` 增强 | 增加运行时网络模块替换（`socket`/`urllib` 强制抛出异常）+ 禁用 `exec`/`eval`/`importlib.import_module` | 📅 S8 |
| 人设可视化编辑 | 用户在设置中通过表单（而非编辑 YAML）修改助理人设：性格描述、语气风格、回复长度偏好等，修改后实时生效 | 📅 S8 |
| 提醒规则配置面板 | 用户在设置中通过可视化界面配置提醒规则：提醒类型、频率、时段偏好、静默期等，无需理解效用函数和权重算法 | 📅 S8 |
| **已落地基础能力** | `check_imports` AST 分层门禁、运行时沙箱最小兜底、pytest 门禁脚本 | 🔶 部分完成 |
...
| **已落地基础能力** | 暂无代码落地；保持为 Sprint 8 排期项 | 🔲 待做 |
