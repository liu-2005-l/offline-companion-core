...
**Persona-constraint output boundary (P3-A4/A5)**: local model candidates entering `PersonaSessionCore` for validated sessions bound to the current release manifest run the P1-frozen L4 scan after generation and arithmetic audit. Identity cliffs and capability/fact denial receive at most one retry with an accuracy-first, persona-honest reminder; a second hit returns deterministic fallback text. User-directed attacks remain observe-only. Validated SSE buffers backend tokens and replays only the audited final reply, so rejected candidates never reach tokens, messages, or memory extraction; unvalidated personas retain immediate streaming. L4 patterns are the eighth source in the release hash chain, while fallback copy lives in a dedicated frozen section of the sixth `reply_copy` source. L4 retry reuses the original immutable `PersonaTurnSignals` and emotion confidence instead of reclassifying same-turn model output. `PersonaL4Trace` is projected to synchronous results, stream terminal events, and assistant-message metadata. Detection expansion, recalibration, cross-turn detection, and low-latency online scanning remain W3 work.
