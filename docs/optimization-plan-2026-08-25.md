...
日期：2026-08-25
定位：接替 optimization-plan-2026-08-23.md（v5）。v5 A/B/C/D 全闭合（D 全链 83f5ca9→b8a8911→8a1bd0c→5045645，998→1080），E 方案出稿待开工（docs/red-team-batch-e-design.md）。本文承接 E 收尾 + 主线回归 + v1.6.0 收尾路径
...
验收：silent 清零 + 全量回归绿 + 新基线 commit 锁定。Batch B 闭合 = v5 原则四判据场收官。
...
依赖图
Batch A（E 清账 + v5 终态对齐）
   └─► Batch B（E-1 侦察 → E-2 修复，依赖 A 的 E-0/C 答案）
          └─► Batch C（6.1-6.6 验证，依赖 E 闭合的稳定基线）
...
机制判据（原则四收官）：Batch B E 红队 silent 清零 + D 工具链生产路径稳定
...
v1.7.0 新能力候选（列清单不立项）：扩展插件生态 / 移动端 / 多用户场景 / UI 自动化 Skill 上线商城——具体方向看 v1.6.0 发布后的用户反馈
非阻断顺手项（搭车）
