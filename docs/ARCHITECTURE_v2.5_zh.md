...
拆解保真校验以无云端为默认场景：用户通过“按/按照/用/使用”指定算法、协议或格式时，LLM 计划必须在至少一个步骤中保留完整约束；首次丢失时仅允许携带定向反馈重拆一次，再次丢失则不创建计划或候选样本，直接回到本地对话并明示方法能力边界。低风险 `chat` 单步计划若标题与移除礼貌包装、方法约束后的原输入高度相似，则按零增值计划降级；工具或 Skill 单步计划不受此规则影响。手动与 Auto 链均传递同一 `fallback_notice`，未配置云端时不报错、不提示配置云端，后续本地回答继续经过算术断言审计。

确定性算法不交给 LLM 推理：当前工具集以本地 `algorithm_booth` 为首个内置工具，识别“按 Booth 算法计算整数乘法”后直接生成两步计划——先执行纯 Python Booth 算法并返回重编码、部分积和寄存器轮次，再由本地模型仅转述工具结果。工具步骤使用固定 `tool_args`，不经样本检索或模型拆解；最终正文仍经过统一算术审计。后续 calculator、CRC、排序和欧几里得工具沿用该“确定性执行 → 语言转述 → 审计对账”边界。
