...
- **临时文件治理**：启动下载器时清理超过 24 小时的 `.gguf.tmp` 残留；近期临时文件保留用于断点续传，避免强制退出后重复消耗带宽。
- **磁盘预检**：开始网络请求前以“模型总大小 − 已下载临时文件大小”计算剩余需求，并与目标目录所在磁盘可用空间比较；空间不足直接进入失败状态，不下载到半途才报错。
...
首次引导由 A1/A2 提供三步状态机：选择推荐模型并下载、设置偏好与称呼、确认完成。引导状态持久化到本地 settings，应用中途关闭后按已保存的 `step` 恢复；老版本存在 `active_model_id` 但没有 `onboarding` 字段时视为已完成，直接进入正常模式。下载 UI 将 `VERIFYING` 显示为“校验中”，避免 100% 后看似卡住。跳过模型下载时仅在存在完整 API Key 且策略允许时进入云端模式，否则明确提示“需要模型或 API key”。引导完成后下次启动进入正常模式。模型缺失、文件被 truncate 或完整性检测失败时，启动必须提示重新下载并沿用 `local → cloud_fallback/no_backend` 降级链，不能静默上云。
...
会话回复在模型生成后执行本地算术断言审计：仅提取十进制裸数字、四则/整数幂与 `=`、`等于`、`（的结果）是` 组成的可机械判定断言，使用 `Decimal` 精确验证。检测到错误时复用一次质量重试槽位并以系统提示尾注反馈正确值；重试不可用或仍失败时保留模型原文并追加确定性警示。流式路径保持 token 输出，终态事件携带审计后的完整正文，桌面端以终态正文替换未审计的临时流式文本；手动与 Auto 计划的 `final_reply` 复用同一审计器，且不回退计划状态机。审计器在 info 级别记录提取数、失败数、重试状态与快路径跳过原因，不记录回复原文。
...
拆解保真校验以无云端为默认场景：用户通过“按/按照/用/使用”指定算法、协议或格式时，LLM 计划必须在至少一个步骤中保留完整约束；首次丢失时仅允许携带定向反馈重拆一次，再次丢失则不创建计划或候选样本，直接回到本地对话并明示方法能力边界。低风险 `chat` 单步计划若标题与移除礼貌包装、方法约束后的原输入高度相似，则按零增值计划降级；工具或 Skill 单步计划不受此规则影响。手动与 Auto 链均传递同一 `fallback_notice`，未配置云端时不报错、不提示配置云端，后续本地回答继续经过算术断言审计。拆解决策在 info 级别记录 R3 入口门控、B4 方法约束命中/丢失、B3 校验拦截与最终放行来源；LLM 拆解原始输出全文保留在 debug 日志，同时以 info 记录字符数、重试状态、样本数与解析/校验结论，便于不翻代码回答“为什么放行或拦截”。

确定性算法不交给 LLM 推理：当前工具集包含本地 `algorithm_booth`、`algorithm_crc32`、`algorithm_gcd`、`algorithm_quicksort` 与 `calculator`。算法请求识别后直接生成两步计划——先执行纯 Python 确定性工具并返回真实中间态，再由本地模型仅转述工具结果；基础算术请求则由 calculator 确定性执行四则或整数幂，并支持中文数字解析。CRC-32 工具固定 UTF-8 编码，自实现按位迭代并在工具本体内与 `zlib.crc32` 交叉验证；欧几里得工具返回辗转相除余数序列；快速排序工具返回 Lomuto 分区快照。工具步骤使用固定 `tool_args`，不经样本检索或模型拆解；最终正文仍经过统一算术审计。
